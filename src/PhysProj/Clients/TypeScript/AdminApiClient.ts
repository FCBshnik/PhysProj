...
export class AdminApiClient {
     ...
     * @return OK
     */
    linkSubWorkAuthorToWork(code: string, authorCode: string): Promise<WorkModel> {
        let url_ = this.baseUrl + "/api/works/{code}/sub-works-authors/{authorCode}";
        if (code === undefined || code === null)
            throw new Error("The parameter 'code' must be defined.");
        url_ = url_.replace("{code}", encodeURIComponent("" + code));
        if (authorCode === undefined || authorCode === null)
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.transformResult(url_, _response, (_response: Response) => this.processLinkSubWorkAuthorToWork(_response));
        });
    }

    protected processLinkSubWorkAuthorToWork(response: Response): Promise<WorkModel> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
     ...
     * @return OK
     */
    unlinkSubWorkAuthorFromWork(code: string, authorCode: string): Promise<WorkModel> {
        let url_ = this.baseUrl + "/api/works/{code}/sub-works-authors/{authorCode}";
        if (code === undefined || code === null)
            throw new Error("The parameter 'code' must be defined.");
        url_ = url_.replace("{code}", encodeURIComponent("" + code));
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.transformResult(url_, _response, (_response: Response) => this.processUnlinkSubWorkAuthorFromWork(_response));
        });
    }

    protected processUnlinkSubWorkAuthorFromWork(response: Response): Promise<WorkModel> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class WorkModel implements IWorkModel {
    ...
    subWorksAuthorsCodes?: string[] | undefined;
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["subWorksAuthorsCodes"])) {
                this.subWorksAuthorsCodes = [] as any;
                for (let item of _data["subWorksAuthorsCodes"])
                    this.subWorksAuthorsCodes!.push(item);
            }
    ...
    toJSON(data?: any) {
        ...
        if (Array.isArray(this.subWorksAuthorsCodes)) {
            data["subWorksAuthorsCodes"] = [];
            for (let item of this.subWorksAuthorsCodes)
                data["subWorksAuthorsCodes"].push(item);
        }
